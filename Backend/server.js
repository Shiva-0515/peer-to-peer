...
//     if (roomSize >= 2) {
//       console.warn(`🚫 Room ${roomId} is full (${roomSize} users)`);
//       socket.emit("room-full", "This room already has 2 participants.");
//       return; // optionally also: socket.disconnect(true);
//     }
...
//       // ✅ Join AFTER validation & capacity check
//       socket.join(roomId);
...
io.on("connection", (socket) => {
  ...
  // ✅ Join Room with max 2 users + JWT check
  socket.on("join-room", ({ roomId, name, token }) => {
    if (!token) {
      console.warn(`🚫 No token provided by ${socket.id}`);
      socket.emit("unauthorized", "Missing authentication token");
    ...
    if (roomSize >= 2) {
      console.warn(`🚫 Room ${roomId} is full (${roomSize} users)`);
      socket.emit("room-full", "This room is Full!.")
      return; // optionally also: socket.disconnect(true);
    }
    ...
    try {
      ...
      socket
        .to(roomId)
        .emit("user-joined", { id: socket.id, name: socket.userName });
      updatePeerList(roomId);
    } catch (err) {
      console.error("❌ Invalid token:", err.message);
      socket.emit("unauthorized", "Invalid or expired token");
